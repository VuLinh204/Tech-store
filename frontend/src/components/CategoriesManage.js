...
const CategoriesManage = () => {
  ...
  // Hàm để gọi API lấy danh sách danh mục từ backend Node.js (chạy trên port 5000)
  const fetchCategories = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/categories");
      setCategories(response.data); // Cập nhật state với dữ liệu nhận được từ Node.js backend
    } catch (error) {
  ...
  // Hàm để xử lý sau khi thêm danh mục thành công
  const handleAddSuccess = () => {
    setIsAdding(false); // Trở về chế độ xem danh sách sau khi thêm thành công
    fetchCategories(); // Lấy lại danh sách danh mục sau khi thêm thành công
  };
